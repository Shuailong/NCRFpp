...
# @Last Modified time: 2018-10-16 19:45:06
...
def recover_label(pred_variable, gold_variable, mask_variable, label_alphabet, word_recover):
    ...
    for idx in range(batch_size):
        pred = [label_alphabet.get_instance(
            pred_tag[idx][idy]) for idy in range(seq_len) if mask[idx][idy] != 0]
        gold = [label_alphabet.get_instance(
...
def recover_nbest_label(pred_variable, mask_variable, label_alphabet, word_recover):
    ...
    for idx in range(batch_size):
        ...
        for idz in range(nbest):
            each_pred = [label_alphabet.get_instance(
...
def batchify_with_label(input_batch_list, device):
    """
        ...
        output:
            ...
            word_seq_tensor: (batch_size, max_sent_len)
            word_seq_lengths: (batch_size,1)
            char_seq_tensor: (batch_size*max_sent_len, max_word_len)
            char_seq_lengths: (batch_size*max_sent_len,1)
    ...
    word_seq_lengths = torch.tensor(
        list(map(len, words)), dtype=torch.long, device=device)
    ...
    for idx in range(feature_num):
        feature_seq_tensors.append(torch.zeros(
            (batch_size, max_seq_len), dtype=torch.long, device=device))
    mask = torch.zeros((batch_size, max_seq_len),
                       dtype=torch.uint8, device=device)
    for idx, (seq, label, seqlen) in enumerate(zip(words, labels, word_seq_lengths)):
        seqlen = seqlen.item()
        word_seq_tensor[idx, :seqlen] = torch.tensor(
            seq, dtype=torch.long, device=device)
        label_seq_tensor[idx, :seqlen] = torch.tensor(
            label, dtype=torch.long, device=device)
        mask[idx, :seqlen] = torch.tensor(
            [1] * seqlen, dtype=torch.uint8, device=device)
        for idy in range(feature_num):
            feature_seq_tensors[idy][idx, :seqlen] = torch.tensor(
                features[idx][:, idy], dtype=torch.long, device=device)
    ...
    char_seq_tensor = torch.zeros(
        (batch_size, max_seq_len, max_word_len), dtype=torch.long, device=device)
    char_seq_lengths = torch.tensor(
        length_list, dtype=torch.long, device=device)
    for idx, (seq, seqlen) in enumerate(zip(pad_chars, char_seq_lengths)):
        for idy, (word, wordlen) in enumerate(zip(seq, seqlen)):
            # print len(word), wordlen
            char_seq_tensor[idx, idy, :wordlen] = torch.tensor(
                word, dtype=torch.long, device=device)
...
def train(data):
    ...
    if data.optimizer.lower() == "sgd":
        ...
        optimizer = optim.Adagrad(
            model.parameters(), lr=data.HP_lr, weight_decay=data.HP_l2)
